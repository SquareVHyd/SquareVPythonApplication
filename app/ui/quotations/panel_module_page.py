...
from app.ui.quotations.module_items_viewer_dialog import ModuleItemsViewerDialog # New import
...
class PanelModulePage(QWidget):
    ...
    def setup_ui(self):
        ...
        self.items_btn = QPushButton("📦 Items") # New button
        self.items_btn.clicked.connect(self._show_module_items_viewer) # Connect to new method
        ...
        header.addWidget(self.items_btn) # Add new button to header
        header.addStretch()
    ...
    def _load_module_types(self):
        """Fetches available module types from the database and configures the delegate for column 6."""
        try:
    ...
    def _render(self, rows):
        ...
        # 3=IngOg, 4=Qty, 6=Module Type, 7=Pole, 8=KA, 9=Release, 10=Protection, 11=Remark
        editable_cols = {3, 4, 6, 7, 8, 9, 10, 11}
    ...
    def add_module(self):
        ...
        if dialog.exec() == QDialog.Accepted:
            try:
                ...
                self._load_module_types() # Sync dropdowns/mapping with potential new types
        ...
        try:
            ...
            if col == 6:
                ...
                if type_id is not None:
                    # Update hidden TypeID column (5) in UI
            ...
            # Basic numeric validation for Qty
            if col == 4:
                try:
                    val = int(val or 1)
                except:
    ...
    def _show_module_items_viewer(self, panel_id=None, pm_id=None):
        if self.quote_id is None:
            QMessageBox.warning(self, "No Quotation Selected", "Please select a quotation first to view module items.")
            return
        
        # Call the show_items method of the main_window (QuotationDetailsPage/Window)
